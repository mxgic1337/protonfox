...
export function checkHover() {
	...
	const mutationObserver = new window.MutationObserver(() => {
		...
		for (const game of games) {
			...
			const hasRating =
				reviews.getElementsByClassName('protonfox-rating-bright').length !== 0;
			if (!hasRating) {
				const gameId = game.id.substring('hover_app_'.length);
				getProtonDBRating(gameId).then(async (rating) => {
					const protonDBRatingTitle = createElement(
						'ProtonDB rating: ',
						['title'],
						'div'
					);
					protonDBRatingTitle.appendChild(
