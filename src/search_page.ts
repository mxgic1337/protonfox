...
function checkSearchResults() {
	...
	for (const game of games) {
		if (game.getElementsByClassName('protonfox-tag').length > 0) {
			continue;
		}
		const name =
			game.getElementsByClassName('platform_img')[0].parentElement ||
			game.getElementsByClassName('title')[0];
		...
		getProtonDBRating(gameId).then(async (rating) => {
			...
			if (status) {
				const statusElement = createElement(
					...
					[
						'protonfox-icon',
						`protonfox-ac-rating-${status.toLowerCase()}`,
						'protonfox-search',
					],
					'span',
					`mask-image: url(${browser.runtime.getURL(`assets/ac-${status.toLowerCase()}.svg`)}); display: inline-block; position: relative; top: -2px;`
				);
