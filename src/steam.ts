...
fetch('https://api.github.com/repos/AreWeAntiCheatYet/AreWeAntiCheatYet/contents/games.json').then(async (res) => {
	if (res.ok) {
		const ghResponse = await res.json() as { content: string };
		antiCheatCompatibility = JSON.parse(atob(ghResponse.content)) as AntiCheatCompatibility[];
	}
	startCheck();
}).catch(err => {
